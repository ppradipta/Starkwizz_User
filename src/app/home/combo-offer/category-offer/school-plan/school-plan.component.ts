...
@Component({
  ...
  styleUrls: ['./school-plan.component.scss'],
})
export class SchoolPlanComponent  implements OnInit {
  ...
  ngOnInit() {
    ...
    this.firestore.collection('price_management').doc('prices').valueChanges().subscribe((prices: any) => {
      if (prices && prices.school) {
        this.payTotal = prices.school;
        this.afterDiscountTotal = prices.school;
      }
    });
  ...
  createSubscription(payment_id: any, index: number, amountText: string) {
    ...
    subscribedSubjects.planCategory = 'School';
