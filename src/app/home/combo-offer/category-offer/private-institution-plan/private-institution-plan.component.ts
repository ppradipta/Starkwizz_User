...
@Component({
  selector: 'app-private-institution-plan',
  templateUrl: './private-institution-plan.component.html',
  styleUrls: ['./private-institution-plan.component.scss'],
})
export class PrivateInstitutionPlanComponent  implements OnInit {
  comboOffer: any = {};
  referralCode: string = '';
  referralUser: any = {};
  ...
  ngOnInit() {
    ...
    // Fetch dynamic price from price_management collection
    this.firestore.collection('price_management').doc('prices').valueChanges().subscribe((prices: any) => {
      if (prices) {
        this.payTotal = prices.privateInstitution;
        this.afterDiscountTotal = prices.privateInstitution;
      }
    });
  ...
  createSubscription(payment_id: any, index: number, amountText: string) {
    ...
    let paymentData = {
      ...
      planCategory: 'PrivateInstitution',
    }
    ...
    subscribedSubjects.planCategory = 'PrivateInstitution';
