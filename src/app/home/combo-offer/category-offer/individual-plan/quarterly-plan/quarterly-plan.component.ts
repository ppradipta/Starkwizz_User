...
@Component({
  ...
  styleUrls: ['./quarterly-plan.component.scss'],
})
export class QuarterlyPlanComponent  implements OnInit {
  ...
  ngOnInit() {
    ...
    this.firestore.collection('price_management').doc('prices').valueChanges().subscribe((prices: any) => {
      if (prices && prices.individual && prices.individual.quarterly) {
        this.payTotal = prices.individual.quarterly;
        this.afterDiscountTotal = prices.individual.quarterly;
      }
    });
